...
> **多图图生图理解**：`/画 图` 多图模式必须在同一条消息里附带多张图片，插件按当前消息图片出现顺序编号为第一张、第二张、第三张……默认最多读取 4 张，可用 `generation_options.image_edit_max_images` 调整。插件会先用 `adapter_openai_chat` 做语义分析，判断是否需要当前消息多图、需要几张图、是否要生成更明确的图生图提示词；多图/编号/参考图语义不会从聊天记录或上一张缓存里拼接图片。如果上游图生图接口支持多图，会把这些图片一起传给接口，否则取决于上游兼容性。

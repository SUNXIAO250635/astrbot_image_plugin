...
> **多图图生图理解**：`/画 图` 多图模式必须在同一条消息里附带多张图片，插件按当前消息图片出现顺序编号为第一张、第二张、第三张……默认最多读取 4 张，可用 `generation_options.image_edit_max_images` 调整。提示词里出现“以第一张为基础、用第二张替换角色特征、第三张参考服装”等描述时，会先用 `adapter_openai_chat` 生成更明确的图生图提示词；多图/编号引用不会从聊天记录或上一张缓存里拼接图片。如果上游图生图接口支持多图，会把这些图片一起传给接口，否则取决于上游兼容性。
...
| `/画 图 <prompt>` | 图生图（普通单图可复用上一张图片；多图必须同条消息附图） | `/画 图 改成水彩风格` |
